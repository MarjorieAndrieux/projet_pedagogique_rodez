...
<!--____________________________Modale: Formulaire modification profil______________________________________________ -->
                    ...
                    <?php
                        if(isset($_POST['validation'])){
                            ...
                            "UPDATE utilisateur SET util_pseudo='".$pseudo_form."', util_avatar='".$avatar_form."', util_notif='".$notif_form."', util_email='".$mail_form."', util_mdp='".$mdp2_form."' WHERE util_email='".$testuser."';");
...
<!--________________________________Carte profil: Script affichage des données_________________________________________-->
                    ...
                    <?php
                        ...
                        "SELECT util_avatar FROM utilisateur WHERE util_email ='".$testuser."';");
                        $resultat_avatar=mysqli_fetch_array($avatar_utilisateur);

                        // Pseudo
                        $identite_utilisateur=mysqli_query($connect, 
                        "SELECT util_pseudo FROM utilisateur WHERE util_email = '".$testuser."';");
...
<!--________________________________Carte profil: Affichage des données_________________________________________-->
                        ...
                        <?php
                            ...
                            INNER JOIN utilisateur ON tag_util.tag_util_id=utilisateur.util_id WHERE util_email='".$testuser."';");
...
<!--________________________________Carte projets: Script d'affichage des projets de l'utilisateur_____________________-->
                        ...
                        //On créé des rangées une fois que 4 projets ont été placés sur la précédente
                            ...
                            echo "<div class='card shadow' data-projet='.$resultat[0].' id='projet_".$resultat[0]."'>
                            <img class='card-img-top img_projets' src='".$resultat[3]."'alt='Image décrivant le projet'>
                            <div class='card-body'>
                                <h5 class='card-title'>".$resultat[1]."</h5>
                                <p class='card-text'>".$resultat[2]."</p>
                            </div>
                            </div>";
                            }
                            $i--;
